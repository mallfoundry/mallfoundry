...
public class DelegatingJpaOrderFactRepository implements OrderFactRepository {
    ...
    @Override
    public OrderFact create(OrderFactKey factKey) {
        return new JpaOrderFact(factKey);
    }

    @Override
    public Optional<OrderFact> findByKey(OrderFactKey factKey) {
        return CastUtils.cast(this.repository.findById(factKey.getKey()));
    }
