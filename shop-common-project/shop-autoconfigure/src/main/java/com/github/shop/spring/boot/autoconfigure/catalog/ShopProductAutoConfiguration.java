...
@Configuration
@EnableConfigurationProperties(ShopProductProperties.class)
public class ShopProductAutoConfiguration {

    @Bean
    @ConditionalOnClass(ProductSearchService.class)
    public ProductSearchService productSearchService(ShopProductProperties properties) {
        ShopProductProperties.Search search = properties.getSearch();
        if (search.getType() == ShopProductProperties.SearchType.LUCENE) {
